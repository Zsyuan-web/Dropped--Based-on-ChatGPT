...
class ChatGPT_rewrite(QThread):
    # 次线程任务
    finished = pyqtSignal(str)  # 自定义信号，表示线程完成任务
    def __init__(self, ip,api_key, message,timeout=30):
        super(ChatGPT_rewrite, self).__init__()
        self.message = message
        self.api_key = api_key
        self.timeout = timeout  # 超时时间，单位为秒
    ...
    def run(self):
        ...
        try:
            ...
            timer.start()  # 启动定时器
            re_text = f"{text}\n"\
                        f"请根据上述总结和关键词进行内容扩写。请将字数限制在{len(self.message)+10}以内。"
            completion = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": f"{re_text}"}]
            )
            text = completion.choices[0].message.content
            # 取消定时器
            timer.cancel()
...
class Dropped_window(QMainWindow):
    def __init__(self):
        ...
        # 主页面”翻译“按钮 链接
        self.ui.pushButton_7.clicked.connect(self.PB_Tr_Rewrite)
        # 主页面”语法检查“按钮 链接
    ...
    def PB_ip_back(self):
        ...
        if ip_text == "无:无" or ip_text == "请输入代理IP:端口" or ip_text == ":":
            ...
            # 写入备用文件
            self.file_handler.write_file(self.content)
            self.ui.lineEdit_2.setText("代理IP已设为备用")
            self.ui.lineEdit_3.setText("并已设置")
            self.ip = ip_text
    ...
    def PB_Rewrite(self):
        ...
        if Rewrite_text != "":
            ...
            reRewrite_text= "请对下面第一段文字进行的改写，使其中的文字最大程度地不同于第二段文字，你只需给出改写后的内容。\n"+Rewrite_text
            self.ChatGPT = ChatGPT_rewrite(self.ip,self.api_key, reRewrite_text)
    ...
    def PB_Fu_Rewrite(self):
        ...
        if Rewrite_text != "":
            ...
            reRewrite_text = "请对下面这段文字进行总结，并给出关键词。\n"+Rewrite_text
            try:
                self.ChatGPT_rewrite = ChatGPT_rewrite(self.ip,self.api_key, reRewrite_text)
                # 次线程任务结束后执行（）内的函数，并将该线程的结果传给该函数
                self.ChatGPT_rewrite.finished.connect(self.on_rewritten)
                self.ChatGPT_rewrite.get_response()  # 次线程任务开始
            except Exception as e:
                print(e)
        else:
            self.ui.textEdit_2.setText("请输入文本……")
    ...
    def PB_Ex_Rewrite(self):
        ...
        if Rewrite_text != "":
            ...
            reRewrite_text = "请对下面这段文字进行扩写。要求：1、不改变这段文字原有的语义；2、只需给出扩写后的内容。\n" +Rewrite_text
    ...
    def PB_Ab_Rewrite(self):
        ...
        if Rewrite_text != "":
            ...
            reRewrite_text = "请对下面这段进行缩写。要求：1、不改变这段文字原有的语义；2、只需给出缩写后的内容。\n" +Rewrite_text
            self.ChatGPT = ChatGPT(self.ip,self.api_key, reRewrite_text)
            # 次线程任务结束后执行（）内的函数，并将该线程的结果传给该函数
            self.ChatGPT.finished.connect(self.on_rewritten)
            self.ChatGPT.get_response()  # 次线程任务开始
    ...
    # 主页面”翻译“按钮 功能
    def PB_Tr_Rewrite(self):
        Rewrite_text = self.ui.textEdit.toPlainText()
        if Rewrite_text != "":
            self.ui.textEdit_2.setText("请稍后，正在进行翻译……")
            reRewrite_text = "请对下面这段文字进行翻译。要求：1、较为正式，论文风格；2、不要口语化；3、如果这段文字内只包含一个词的话，请给这个词的多个翻译结果；4、如果存在翻译结对其进行标号，并且第一个是在科技论文中最为常用的。\n" +Rewrite_text
            self.ChatGPT = ChatGPT(self.ip, self.api_key, reRewrite_text)
            # 次线程任务结束后执行（）内的函数，并将该线程的结果传给该函数
            self.ChatGPT.finished.connect(self.on_rewritten)
            self.ChatGPT.get_response()  # 次线程任务开始
    ...
    def PB_Gr_Rewrite(self):
        ...
        if Rewrite_text != "":
            ...
            reRewrite_text = "请对下面这段文字进行语法检查和错别字检查，并给出修改后的句子和修改建议。\n" +Rewrite_text
            self.ChatGPT = ChatGPT(self.ip,self.api_key, reRewrite_text)
            # 次线程任务结束后执行（）内的函数，并将该线程的结果传给该函数
            self.ChatGPT.finished.connect(self.on_rewritten)
            self.ChatGPT.get_response()  # 次线程任务开始
